...
func (s *Server) serverTCP(option *options, conn *net.TCPConn) {
	...
	if proto, err = Set(); err == nil {
		//log.LogFile.I("proto :%s", proto)
		//连接合法性校验

		if Key, err = s.AuthTCP(ctx, proto, ch); err == nil {
			Online(proto, ch)
		}
	}
	if err != nil {
	...
	for {
		...
		if proto.Opr != grpc.OpAuth {
			if err := Operate(ctx, proto, ch); err != nil {
				break
			}
		}
...
func (s *Server) dispatchTCP(conn *net.TCPConn, ch *Channel) {
	...
	for {
		...
		case grpc.ProtoReady: //客户端发送过来的消息
			...
			if channel := Channel(info.Rid); channel != nil {
				err = Push(msg)
				if err != nil {
			...
			//将消息发送给自己
			if err = msg.WriteTCP(&Writer); err != nil {
				goto field
			}
			msg.Body = nil
			getAdv()
