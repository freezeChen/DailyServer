...
func (p *Proto) ReadTCP(r *bufio.Reader) (err error) {
	...
	Ver = int32(binary.BigEndian.Uint16(headBuf[VerOffset:OperationOffset]))
	Opr = int32(binary.BigEndian.Uint32(headBuf[OperationOffset:SeqIdOffset]))
	Id = int32(binary.BigEndian.Uint32(headBuf[SeqIdOffset:]))
...
func (p *Proto) WriteTCP(w *bufio.Writer) (err error) {
	...
	packLen = uint32(RawHeaderSize) + uint32(len(Body))

	binary.Write(w, binary.BigEndian, packLen)
	binary.Write(w, binary.BigEndian, uint16(RawHeaderSize))
	binary.Write(w, binary.BigEndian, uint16(Ver))
	binary.Write(w, binary.BigEndian, uint32(Opr))
	binary.Write(w, binary.BigEndian, uint32(Id))
	binary.Write(w, binary.BigEndian, Body)
...
func (p *Proto) ReadWebSocket(ws *websocket.Conn) (err error) {
	...
	Ver = int32(binary.BigEndian.Uint16(allBuf[VerOffset:OperationOffset]))
	Opr = int32(binary.BigEndian.Uint32(allBuf[OperationOffset:SeqIdOffset]))
	Id = int32(binary.BigEndian.Uint32(allBuf[SeqIdOffset:]))
	...
	if bodyLen = packLen - uint32(headerLen); bodyLen > 0 {
		...
		Body = allBuf[headerLen:packLen]
		glog.Info("websocket msg:", string(Body))
	} else {
		Body = nil
	}
	return nil
...
func (p *Proto) WriteWebSocket(ws *websocket.Conn) (err error) {
	...
	binary.BigEndian.PutUint16(buf[VerOffset:], uint16(Ver))
	binary.BigEndian.PutUint32(buf[OperationOffset:], uint32(Opr))
	binary.BigEndian.PutUint32(buf[SeqIdOffset:], uint32(Id))

	fmt.Println(packLen, len(Body), len(buf))
