...
// value + key -> MD5加密
func GetMd5StringByKey(value string, key string) string {
	dst := []byte(key)
	...
	des2 := fmt.Sprintf("%X", dst)[:8]

	crypted, err := TripleDesEncrypt([]byte(value), []byte(des2))
	if err != nil {
		return ""
	}

	h.Reset()
	io.WriteString(h, fmt.Sprintf("%x", crypted))
